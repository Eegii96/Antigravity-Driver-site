...
export default function Footer() {
  ...
  return (
    <footer id="footer-section" className="bg-[var(--color-brand-bg2)] text-slate-400 border-t border-[var(--color-glass-border)] text-sm py-12 px-6 relative">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        <div>
          <div className="flex items-center space-x-2 text-[#f1f3f8] font-semibold mb-4 text-base">
            <div className="w-8 h-8 rounded-lg bg-[#26282d] border border-violet-500/30 flex items-center justify-center relative overflow-hidden shrink-0 shadow-md">
        ...
        <div>
          <h4 className="text-[#f1f3f8] font-medium mb-4 text-sm font-sans">Аюулгүй байдлын зөвлөгөө</h4>
        ...
        <div>
          <h4 className="text-[#f1f3f8] font-medium mb-4 text-sm font-sans">Холбоо барих</h4>
      ...
      <div className="max-w-7xl mx-auto border-t border-[var(--color-glass-border)] mt-8 pt-6 flex flex-col md:flex-row items-center justify-between text-xs text-slate-400 font-sans">
        <p>© {new Date().getFullYear()} Хүнд машин, механизм & Газар шорооны ажлын сайт. Бүх эрх хуулиар хамгаалагдсан.</p>
        <div className="flex space-x-6 mt-4 md:mt-0">
          <span 
            onClick={() => setShowTerms(true)}
            className="hover:text-[#f1f3f8] cursor-pointer transition-colors"
          >
            Үйлчилгээний нөхцөл
          </span>
          <span 
          ...
          <span className="text-slate-400">Хувилбар 1.2.0</span>
        </div>
      </div>
      ...
      {showTerms && (
        ...
        >
          <div 
            ...
            className="bg-slate-900/60 backdrop-blur-xl border border-[var(--color-glass-border)] rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl relative"
          >
            {/* Header */}
            <div className="sticky top-0 bg-slate-900/80 backdrop-blur-xl border-b border-[var(--color-glass-border)] px-6 py-4 flex items-center justify-between z-10">
              ...
              <button 
                onClick={() => setShowTerms(false)}
                className="text-slate-400 hover:text-[#f1f3f8] transition-colors cursor-pointer p-1 rounded-lg hover:bg-white/10"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {/* Footer */}
            <div className="border-t border-[var(--color-glass-border)] px-6 py-4 flex justify-end bg-slate-900/60">
              <button 
                onClick={() => setShowTerms(false)}
                className="bg-violet-600 hover:bg-violet-500 text-white font-semibold px-5 py-2 rounded-xl transition-all cursor-pointer text-xs font-sans"
              >
                Ойлголоо, зөвшөөрөв
              </button>
            </div>
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-5 text-xs text-slate-300 leading-relaxed font-sans">
              ...
              <div className="space-y-2">
                <h4 className="font-bold text-[#f1f3f8] text-sm">1. Цуглуулах мэдээлэл</h4>
            ...
            <div className="border-t border-[var(--color-glass-border)] px-6 py-4 flex justify-end bg-slate-900/60">
              <button 
                onClick={() => setShowPrivacy(false)}
                className="bg-cyan-500 hover:bg-cyan-400 text-white font-semibold px-5 py-2 rounded-xl transition-all cursor-pointer text-xs font-sans"
              >
